...
# TODO / Note for John: I think IPOPTLike only has an rtol, philosophically speaking
constrained_minimisers = (optx.IPOPTLike(rtol=1e-3, atol=1e-3),)
...
def test_runtime_unconstrained_minimisers(benchmark, minimiser, problem):
    ...
    compiled = eqx.filter_jit(
        eqx.Partial(
            optx.minimise,
            benchmark_problem.objective,
            minimiser,
            benchmark_problem.y0(),
            benchmark_problem.args(),
        )
    )
...
def test_runtime_constrained_minimisers(benchmark, minimiser, problem):
    benchmark_problem = problem()
    compiled = eqx.filter_jit(
        eqx.Partial(
            optx.minimise,
            ...
            benchmark_problem.y0(),
            benchmark_problem.args(),
            constraint=benchmark_problem.constraint,
            bounds=benchmark_problem.bounds(),
        )
    )

    def wrapped():
        return block_tree_until_ready(compiled())
