...
from .._custom_types import Aux, ConstraintOut, Out, Y
...
# TODO(jhaffner): solve the linear system in a smarter way, e.g. by using the block
# treatment as in qpax.


def _interior_step(
    y: Y, dual: ConstraintOut, f_info: FunctionInfo, linear_solver, centrality
) -> tuple[tuple[Y, ConstraintOut, ConstraintOut], RESULTS]:
    ...
    if f_info.constraint_jac is None:
        raise ValueError("Interior step requires a constraint Jacobian.")
    if f_info.constraint_residual is None:
        raise ValueError("Interior step requires a constraint residual.")

    else:
        slack = f_info.constraint_residual
...
class _InteriorDescentState(eqx.Module, Generic[Y, ConstraintOut], strict=True):
    step: Y
    dual: ConstraintOut  # Storing dual in descent state: only with LearningRate(1.0)
...
class InteriorDescent(
    ...
    def init(
        ...
        dual = tree_full_like(f_info_struct.constraint_residual, 1.0)
        # TODO: hard coded value for centrality? Initial value only? 0.9
    ...
    def query(
        ...
        # TODO(jhaffner): optional improvement: IPOPT uses linear least squares fit to
        # initialise the dual variables. We could do that here on the first step, which
        # would just require keeping an extra boolean in the state and using it to
        # decide whether to initialise the duals in here or not.

        steps, result = _interior_step(
            y, state.dual, f_info, self.linear_solver, state.centrality
        )
        feasible_steps = _truncate_to_feasible(
            steps, state.dual, f_info.constraint_residual, self.boundary_offset
        )
    ...
    def step(self, step_size: Scalar, state) -> tuple[Y, RESULTS]:
