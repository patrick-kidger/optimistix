...
from .._custom_types import Constraint, ConstraintOut, Y
...
def _make_objective(y, constraint, penalty_parameter):
    ...
    def objective(_y, args):
        ...
        residual = jnp.where(residual < 0, -residual, 0)
        # TODO: enable usage of other kinds of norms
        return two_norm(residual) + penalty_parameter * squared_scaled_deviation(_y)
...
class ClosestFeasiblePoint(AbstractBoundaryMap, strict=True):
    ...
    def __call__(
        ...
        del bounds  # TODO: bounds currently only included as inequality constraints
        ...
        if constraint is None:
            ...
            sol = minimise(objective, self.solver, y, throw=False)
