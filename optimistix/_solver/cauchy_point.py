...
def _boundary_intercepts(y: Y, f_info: FunctionInfo.EvalGradHessian):
...
def cauchy_point(y: Y, f_info: FunctionInfo.EvalGradHessian) -> Y:
    ...
    if f_info.bounds is None:
        ...
        if isinstance(f_info, FunctionInfo.EvalGradHessian):
            ...
            # taking a step unless the gradient is zero.
...
def cauchy_newton_step(y, f_info, linear_solver):
    ...
    # Initialise dual values: grad(f) = lambda * grad(c), where c is the constraint
    # function. Since we have bound constraints, grad(c) is one where the constraint is
    # active and zero where it is not.
    dual = tree_where(active, f_info.grad, tree_full_like(y, 0.0))
