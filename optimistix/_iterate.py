...
class AbstractIterativeSolver(
    AbstractSolver[Y, Out, Aux, SolverState], Generic[Y, Out, Aux, SolverState]
):
    ...
    def solve(
        ...
        **Returns:**
        ...
        inputs = self, fn, args, y0, options, max_steps, f_struct, aux_struct, tags
        out, (num_steps, result, final_state, aux) = adjoint.apply(
            _iterate, self.rewrite_fn, inputs, tags
        )
        stats = {"num_steps": num_steps, "max_steps": max_steps}
        sol = Solution(
            value=out, result=result, state=final_state, aux=aux, stats=stats
        )
        if throw:
    ...
    @staticmethod
    @abc.abstractmethod
    def rewrite_fn(root, residual, inputs, /) -> PyTree:
        ...
...
def _iterate(inputs, while_loop):
    solver, fn, args, y0, options, max_steps, f_struct, aux_struct, tags = inputs
    del inputs
