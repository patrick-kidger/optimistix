...
class Bisection(AbstractRootFindSolver):
  ...
  def init(self, root_fn, y: Scalar, args, options):
    upper = options["upper"]
    lower = options["lower"]
  ...
  def step(self, root_fn, y: Scalar, args, state):
    del y
    new_y = state.lower + 0.5 * (state.upper - state.lower)
    error = root_fn(new_y, args)
  ...
  def terminate(self, root_fn, y: Scalar, args, state):
    del root_fn, y, args
