class QR(AbstractLinearSolver):
  ...
  def compute(self, state, vector, options):
    ...
    if transpose.value:
      ...
      solution = q @ solve_triangular(r.T, vector, lower=False, unit_diagonal=False, rcond=self.rcond)
    else:
      # Least squares solution if ill-posed
      solution = solve_triangular(r, q.T @ vector, lower=False, unit_diagonal=False, rcond=self.rcond)
